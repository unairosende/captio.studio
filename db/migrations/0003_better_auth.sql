-- Better Auth schema: users, sessions, accounts, verification, and the
-- organisation plugin's organization / member / invitation tables.
--
-- Generated, not hand-written:
--   node --env-file=.env.local db/auth-schema.ts
--
-- It is checked in rather than applied by Better Auth's own migrator so that
-- db/migrations stays the single ordered history of this database. Two things
-- migrating the same database is how schemas drift apart between environments.
--
-- Do not edit this file. Change lib/auth/server.ts, re-run the generator, and
-- put the difference in a new numbered migration.
